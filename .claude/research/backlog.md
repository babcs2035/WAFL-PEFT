...
**未決（人間判断待ち）**: なし．B23 で次イテレーションの方針が人間により確定した．
**再開時はまず B23（Iter26 方針・最新）→ B22（Iter26 方針・旧）→ B21（Iter26 方針・旧）→ B20（Iter25 方針）→ B19（Iter23 方針）→ B18（Iter22 方針）→ B16（Iter21 方針）→ B13（次セッションへの申し送り）の順に読むこと．**

---

## B23 [resolved-by-human 2026-08-08] Iter26: 評価専用ホスト .101/.104 を学習ノードに転用し 5ノード復帰

- **状況**: B22 は wafl508/.109 の NVML ドライバーミスマッチが未解消のため 3ノード
  （`.100/.102/.103`）で W4 baseline 対比実験を継続する方針だった。この後の調査で，
  評価専用ホスト `.101`/`.104` は NVML 障害の対象ではなく健全に動作することを確認し，
  これらを学習ノードに転用すれば NVML 修復を待たずに 5ノード構成へ復帰できると判明した。
- **自動選択**: `config/hosts.txt` を `.100/.102/.103` → `.100/.101/.102/.103/.104`（5ノード）へ変更。
  `config/hosts.eval.txt` を `.101/.104/.105/.106/.107` → `.105/.106/.107/.108/.109`
  （転用した `.101`/`.104` を外し，空いた `.108`/`.109` を評価専用ホストへ追加）。
  `config/settings.json` の `contact_pattern_file` を n=3 → n=5 版（`rwp_n05_a0500_r100_p10_s42.json`）
  へ更新。
- **人間の判断（2026-08-08，md/Notion 整理セッション）**: config 変更時点でローカルとサーバー
  （wafl-ctrl5 に deploy 済みの `.100/.102/.103/.108/.109` 版）が食い違っていたため確認したところ，
  **ローカルの `.101/.104` 転用案を正とする**と回答があった。サーバー側の deploy 反映は次回の
  実験投入時（`mise setup && mise deploy`）に行う。
- **根拠**: `.108`/`.109` の NVML 修復時期は不確定なままだが，`.101`/`.104` は即使用可能。
  接触パターン n=5 に戻せるため，Iter24 以前（n=5）との比較可能性も回復する。
- **要レビュー**: この変更はまだ実験投入されていない（`docker ps` 空，`results/` に Iter26
  ディレクトリなし）。次イテレーションの実験フェーズで `mise deploy` を忘れずに実行すること。

---

## B22 [auto-decided 2026-08-08] Iter26: W4 baseline 対比実験 — 3ノード構成で control vs treatment（B23 により方針転換）

- **状況**: Iter25 で maxsize=8192→65536 の修正が有効だったことを確認。`_final.log` が全3peerで生成された。ただし3ノード構成（wafl508/509 NVMLエラー）での実験のため、n=3接触パターンとなり過去実験との accuracy 比較は不可。
- **B21との関係**: B21は「5ノード復活がプリ条件」としたが、NVML修復の目処は立っていない。B21の「3ノード継続時はやむを得ず実施」という代替案を今回採用する。

- **自動選択**: Iter26 で W4 なし baseline 対比実験を3ノード構成で実施する。
  - control: `WAFL_SKIP_LOCAL_TRAIN_WHEN_ISOLATED=0`
  - treatment: `WAFL_SKIP_LOCAL_TRAIN_WHEN_ISOLATED=1`
  - 同一接触パターン、同一ノード構成で2回実験し、loss/throughput/stall/contact を比較。

- **ノード数判断の根拠**:
  - 5ノード: NVML修復待ち（時期不確定）。比較性は保てるが即着手不可。
  - 3ノード: 即着手可能。同一構成内での W4 効果測定は可能。ただし n=3 接触パターンは原典 n=10 と異なるため、accuracy の絶対値は過去実験と異なる。W4 の独自効果を loss/throughput で測定できる。

- **固定構成**:
  - `max_seq_len=320`、3 ノード（`.100/.102/.103`）
  - `sample_limit=500`、`WAFL_SELF_EVAL=0`、`WAFL_MERGE_INCLUDE_SELF=1`
  - `maxsize=65536`（Iter25 実装分）
  - 接触パターン n=3（`rwp_n03_a0500_r100_p10_s42.json`）

- **変更ファイル**: なし（コード変更は Iter24/25 で完了）。settings.json の experiment_name のみ。

- **根拠**: メトリクスバグは解消済み（Iter25 実装分）。W4 の独自効果を loss/throughput/stall で測定する必要がある。

- **要レビュー**:
  - 3ノードでの対比実験は、過去の実験（n=5）との accuracy 比較はできないが、同一構成内での W4 効果測定は可能。
  - 5ノードへの復帰が確認できたら、その時点で 5ノードでの再測定へ切り替える。

---

## B21 [auto-decided 2026-08-08] Iter26: W4 継続 — baseline 対比実験（5ノード復活がプリ条件）

- **状況**: Iter25 で W4 (`skip_local_train_when_isolated`) のメトリクスバグ修正（maxsize=8192→65536）が有効だったことを確認。`_final.log` が全3peerで生成され、995行のメトリクス（loss, throughput, stall_duration, contact_events）が取得可能になった。
- **ただし**: 3ノード構成（wafl508/509 NVMLエラー）では W4 の独自効果を判定するには不十分。接触パターンが n=3 に変化し、過去実験（n=5）との比較性が損なわれる。

- **自動選択**: Iter26 で W4 なし baseline 対比実験を実施する。**ただしプリ条件として wafl508/509 の NVML 修復が必要**。
  - 修復不可の場合は、3ノード構成での対比実験（n=3接触パターン）をやむを得ず実施。ただし結果の解釈には注意。

- **固定構成（5ノード復活時）**:
  - `max_seq_len=320`、5 ノード（`.100/.102/.103/.108/.109`）
  - `sample_limit=500`、`WAFL_SELF_EVAL=0`、`WAFL_MERGE_INCLUDE_SELF=1`
...
- **固定構成（3ノード継続時）**:
  - `max_seq_len=320`、3 ノード（`.100/.102/.103`）
  - 接触パターン n=3（`rwp_n03_a0500_r100_p10_s42.json`）
  - 同上の control/treatment 対比

- **変更ファイル**: なし（コード変更は Iter24/25 で完了）。settings.json の experiment_name のみ。

- **根拠**: B20 で計画された W4 なし baseline 対比実験の次のステップ。メトリクスバグは解消済み。

- **要レビュー**: 
  - wafl508/509 の NVML 修復状況。修復不可の場合は 3ノード構成での実験を承認すること。
  - 3ノードでの対比実験は、過去の実験（n=5）との accuracy 比較はできないが、同一構成内での W4 効果測定は可能。
